...
class CameraPosePublisherNode(Node):
    ...
    def publish_joint_states(self):
        q1, q2 = self.camera_poses.get_matches(old_frame, new_frame)
        if q1 is not None and len(q1) > 20 and len(q2) > 20:
            ...
            joint_state_msg.position = [cur_pose[1, 3], cur_pose[2, 3], cur_pose[0, 3]]
...
def main(args=None):
    ...
    skip_frames = 2
    data_dir = ''
    camera_pose_publisher_node = CameraPosePublisherNode(data_dir, skip_frames, intrinsic)
    rclpy.spin(camera_pose_publisher_node)
    camera_pose_publisher_node.destroy_node()
    rclpy.shutdown()
