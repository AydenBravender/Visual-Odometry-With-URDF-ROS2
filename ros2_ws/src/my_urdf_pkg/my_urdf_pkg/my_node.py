...
class CameraPoses():
    ...
    def decomp_essential_mat_old(self, E, q1, q2):
        ...
        if np.isnan(P).any() or np.isinf(P).any():
            print("Invalid values detected in the projection matrix.")
            print("self.K:", self.K)
            print("T:", T)
            print("Result of matmul:", P)
...
class CameraPosePublisherNode(Node):
    def __init__(self, data_dir, skip_frames, intrinsic, cur_pose):
        super().__init__('camera_pose_publisher_node')
        self.publisher_ = self.create_publisher(JointState, 'joint_states', 10)
        self.timer = self.create_timer(1.0 / 10, self.publish_joint_states)
        self.camera_poses = CameraPoses(data_dir, skip_frames, intrinsic)
        self.cur_pose = cur_pose  # Initialize cur_pose in the class
    ...
    def publish_joint_states(self):
        ...
        if self.process_frames and ret:
            q1, q2 = self.camera_poses.get_matches(self.old_frame, self.new_frame)
            if q1 is not None and len(q1) > 20 and len(q2) > 20:
                transf = self.camera_poses.get_pose(q1, q2)
                self.cur_pose = self.cur_pose @ transf  # Update cur_pose
                joint_state_msg = JointState()
                joint_state_msg.header.stamp = self.get_clock().now().to_msg()
                joint_state_msg.name = ['X', 'Y', 'Z']
                # order is -2,0,1
                joint_state_msg.position = [
                    -(self.cur_pose[2, 3]), self.cur_pose[0, 3], self.cur_pose[1, 3]
                ]
                self.publisher_.publish(joint_state_msg)

        elif self.process_frames and ret is False:
            self.cap.release()
            self.destroy_node()
    ...
    def run_node(self):
        rclpy.spin_once(self, timeout_sec=1.0 / 10)

def main(args=None):
    rclpy.init(args=args)
    with open('/home/bravenderbros/ros2_ws/src/my_urdf_pkg/intrinsicNew.npy', 'rb') as f:
        intrinsic = np.load(f)
    skip_frames = 2
    data_dir = ''
    ...
    camera_pose_publisher_node = CameraPosePublisherNode(data_dir, skip_frames, intrinsic, start_pose)
    while rclpy.ok():
        camera_pose_publisher_node.run_node()
    rclpy.shutdown()
